use crate::user::{
    User, UserEntry, USER_ANCHOR_TEXT, USER_ANCHOR_TYPE, USER_ENTRY_NAME, USER_LINK_TYPE,
};
use hdk::{
    error::ZomeApiResult,
...
pub fn delete_user(id: Address) -> ZomeApiResult<Address> {
...
pub fn list_users() -> ZomeApiResult<Vec<User>> {
    hdk::get_links_and_load(
        &user_anchor()?,
        LinkMatch::Exactly(USER_LINK_TYPE),
        LinkMatch::Any,
    )
    .map(|user_list| {
        user_list
...
pub fn update_user(id: Address, user_input: UserEntry) -> ZomeApiResult<User> {
    ...
    hdk::update_entry(
        Entry::App(USER_ENTRY_NAME.into(), user_input.clone().into()),
        &address,
    )?;
